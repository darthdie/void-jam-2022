...
const layer = createLayer(id, () => {
    ...
    const conversion = createCumulativeConversion(() => ({
        ...
        onConvert: (amountGained) => {
            console.log(format(amountGained));
    ...
    const loopGain = computed(() => {
        ...
        if (unref(milestone3.earned.value)) {
            gain = gain.add(Decimal.mul(loop.value, 0.05));
        }

        if (Decimal.gte(knowledge.loopBoost.value, 1)) {
            gain = gain.times(knowledge.loopBoost.value);
        }
    ...
    const milestone3 = createMilestone(() => ({
        shouldEarn() {
            return Decimal.gte(loopResets.value, 100);
        },
        display: {
            requirement: "100 Total Loop Resets",
            effectDisplay: `"Rinse and repeat, rinse & repeat, rinse y repeat"<br\>Loops boost their own base gain.`
        }
    }));
            ...
            tab: createTab(() => ({
                display: jsx(() => (
                    <>
                        ...
                        {Decimal.gt(loopGain.value, 0) ? <div>({format(loopGain.value.floor())} per reset)</div> : null}
